...
Pitch_Detector::Pitch_Detector(const int &step, const int &window_size)
{
...
void Pitch_Detector::setWave(const std::vector<real> &wave)
{
    this->wave.clear();
    for(auto it = wave.begin(); it != wave.end(); it++)
        this->wave.push_back(*it);
    this->num_of_samples = (int)wave.size();
}

void Pitch_Detector::getWave(std::vector<real> &wave)
{
    for(auto it = this->wave.begin(); it !=this-> wave.end(); it++)
        wave.push_back(*it);
}

std::vector<std::string> Pitch_Detector::get_notes()

{
    vector<string> notes;
    for(auto it = this->freq.begin(); it != this->freq.end(); it++)
        notes.push_back(this->pitch_pairs[*it]);

    return notes;

}
real Pitch_Detector::find_note(string note)
{
    for(const auto &[key, val] : this->pitch_pairs)
    {
        if(note == val) return key;
    }

    return 0;
}

void Pitch_Detector::load(const std::string &filename)
{
